...
/*
	When creating the hash-map, need to know something about what is going
	in it.  
		...
		int size)
{
	...
	hashmap->entry = (void *)malloc((hashmap->record.key_size + hashmap->record.value_size + 1)*hashmap->map_size);
	hashmap->compute_hash = (*f);			//Allows for binding of different hash functions
	...
	for (i =0; i < size; i++)
	{
		((hash_bucket_t *)(hashmap->entry + ((hashmap->record.key_size+hashmap->record.value_size + SIZEOF(STATUS))*i)))->status = EMPTY ;
	}
	return 0;
}
		...
		char * value)
{
	...
	{
		item = ((hash_bucket_t *)((hash_map->entry+(hash_map->record.key_size+hash_map->record.value_size + SIZEOF(STATUS))*loc)));
		...
		if (item->status == IN_USE) 	//if a cell is in use, need to key to
		{
			if (memcmp(item->data,key,hash_map->record.key_size) == IS_EQUAL)
			{
				...
				{
					memcpy(item->data+hash_map->record.key_size, value, (hash_map->record.value_size));
					return err_ok;
				}
				else
				{
		...
		else if (item->status == EMPTY || item->status == DELETED)
		{
			...
			{
				int i;
				for (i = 0; i< (hash_map->record.key_size+hash_map->record.value_size);i++)
		...
		char * key)
{
	...
	while (count != hash_map->map_size)
	{
		...
		// locate first item
		hash_bucket_t * item = (((hash_bucket_t *)((hash_map->entry+(hash_map->record.key_size+hash_map->record.value_size + SIZEOF(STATUS))*loc))));
		...
		else //calculate if there is a match
		{
			if (item->status != DELETED)
			{
				...
				int key_is_equal = memcmp(item->data,key,hash_map->record.key_size);
		...
		char ** data)
{
	...
	if (loc != -1)
	{
		...
		int data_length = hash_map->record.key_size+hash_map->record.value_size;
	...
	for (i=0; i < size; i++)
	{
		io_printf("%d -- %i ", i, ((hash_bucket_t *)((hash_map->entry+(record->key_size+record->value_size +  SIZEOF(STATUS))*i)))->status);
		{
			...
			else
			{
				...
				{
					io_printf("%X ",*(unsigned
							char *)(((hash_bucket_t *)((hash_map->entry+(record->key_size+record->value_size + SIZEOF(STATUS))*i)))->data+j));
				}
			}
			io_printf("\n");
		}
