...
err_t oah_destroy(hashmap_t * hash_map)
{
...
int oah_getLocation(hash_t num, int size)
{
	return num % size;
}

err_t oah_update(hashmap_t * hash_map, char * key, char * value)
{
...
err_t oah_insert(hashmap_t * hash_map, char * key, char * value)
{
	int i;
...
int oah_findItemLoc(hashmap_t * hash_map, char * key)
{
	hash_t hash = hash_map->compute_hash(hash_map, key,
	        hash_map->record.key_size);	//compute hash value for given key
...
err_t oah_delete(hashmap_t * hash_map, char * key)
{
...
err_t oah_query(hashmap_t * hash_map, char * key, char ** data)
{
	int loc = oah_findItemLoc(hash_map, key);
...
void oah_print(hashmap_t * hash_map, int size, record_t * record)
{
	int i;
		                        ...
		                        + SIZEOF(STATUS)) * i)))->status);
		{
			...
			else
			{
				...
				for (j = 0; j < (record->key_size + record->value_size); j++)
				{
					io_printf("%X ",
					        *(unsigned
					        char *)(((hash_bucket_t *)((hash_map->entry
					                + (record->key_size + record->value_size
...
hash_t oah_compute_simple_hash(hashmap_t * hashmap, char * key, int size_of_key)
{
