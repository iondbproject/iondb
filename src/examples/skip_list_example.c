...
main(
	...
	dictionary_create(&skip_handler, &test_dict, 1, key_type, key_size, value_size, maxheight);
	...
	while ((in = getchar()) != 'X') {
		...
		switch (in) {
			...
			case 'F': {
				...
				sl_node_t *node = sl_find_node((skiplist_t *) test_dict.instance, (ion_key_t) &key);
