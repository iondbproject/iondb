...
void setup()
{
    ...
    /* Create Open Address Hash dictionary instance: key type, key size, value size, dictionary size */
    Dictionary<string> *oah = new OpenAddressHash<string>(key_type_char_array, sizeof(int), 10, 20);

    /* Insert into open address hash dictionary */
    Serial.println("Insert key_1 [1]");
    string key_1 = "key_1";
    int value = 1;
    oah->insert(key_1, value);

    /* Return the value inserted into the open address hash dictionary */
    int returned_value = oah->get<int>(key_1);
    ...
    /* Update value */
    int new_value = 2;
    oah->update(key_1, new_value);

    /* Verify update was successful */
    returned_value = oah->get<int>(key_1);
    ...
    int value_1 = 100;
    oah->insert(key_1, value_1);

    Serial.println("Insert key_1 [200]");
    int value_2 = 200;
    oah->insert(key_1, value_2);

    Serial.println("Insert key_2 [300]");
    string key_2 = "key_2";
    int value_3 = 300;
    oah->insert(key_2, value_3);

    /* Perform equality query on the key of 3 */
    Serial.println("Testing equality query on key key_1: ");
    dict_cursor_t *cursor = oah->equality(key_1);
    ion_record_t record = oah->getRecord(cursor);
    ...
    Serial.println("End OpenAddressHash (string key) test.");
