...
void setup()
{
    ...
    /* Create Open Address File Hash dictionary instance: key type, key size, value size, dictionary size */
    Dictionary<string> *oafh = new OpenAddressFileHash<string>(key_type_char_array, sizeof(int), 10, 20);

    /* Insert into open address file hash dictionary */
    Serial.println("Insert key_1 [1]");
    string key_1 = "key_1";
    int value = 1;
    oafh->insert(key_1, value);

    /* Return the value inserted into the open address file hash dictionary */
    int returned_value = oafh->get<int>(key_1);
    ...
    /* Update value */
    int new_value = 2;
    oafh->update(key_1, new_value);

    /* Verify update was successful */
    returned_value = oafh->get<int>(key_1);
    ...
    int value_1 = 100;
    oafh->insert(key_1, value_1);

    Serial.println("Insert key_1 [200]");
    int value_2 = 200;
    oafh->insert(key_1, value_2);

    Serial.println("Insert key_2 [300]");
    string key_2 = "key_2";
    int value_3 = 300;
    oafh->insert(key_2, value_3);

    /* Perform equality query on the key of 3 */
    Serial.println("Testing equality query on key key_1: ");
    dict_cursor_t *cursor = oafh->equality(key_1);
    ion_record_t record = oafh->getRecord(cursor);
    ...
    Serial.println("End OpenAddressFileHash (string key) test.");
