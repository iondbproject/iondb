...
quick_sort_int_comparator(
	...
	int result = (*((int *) value_a) - *((int *) value_b));
...
quick_sort_assert_sorted_order(
	...
	for (i = 0; i < num_values - 1; i++) {
		ion_comparison_e comp_result = quick_sort_int_comparator(NULL, ((char *) data) + value_size * i, ((char *) data) + value_size * (i + 1));
		PLANCK_UNIT_ASSERT_TRUE(tc, less_than == comp_result || equal == comp_result);
	}
}
...
quick_sort_verify_result(
	...
	ion_in_memory_sort_init(&ims, data, num_values, value_size, context, compare_function, ION_IN_MEMORY_SORT_QUICK_SORT);
	PLANCK_UNIT_ASSERT_INT_ARE_EQUAL(tc, err_ok, ion_in_memory_sort_run(&ims));
...
test_quick_sort_single(
	...
	quick_sort_verify_result(tc, data, expected, sizeof(data) / sizeof(int), sizeof(int), NULL, quick_sort_int_comparator);
}
