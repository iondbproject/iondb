...
test_cpp_wrapper_insert_delete(
	...
	for (int i = 0; i < 10; i++) {
		...
		PLANCK_UNIT_ASSERT_INT_ARE_NOT_EQUAL(tc, i * 2, ret_val);
	}
}
...
test_cpp_wrapper_all_records(
	...
	/* Check that same number of records are found as were inserted with desired key. */
	PLANCK_UNIT_ASSERT_INT_ARE_EQUAL(tc, 10, total_records);
...
test_cpp_wrapper_all_records_on_all_implementations(
	...
	dict = new BppTree<int, int>(key_type_numeric_signed, sizeof(int), sizeof(int));
	test_cpp_wrapper_all_records(tc, dict);
	delete dict;
...
test_cpp_wrapper_open_close(
	...
	status = dict->insert(66650, GTEST_DATA);
...
//	int dict_size = dict->dict_size;
	...
	char *ret_val = dict->get(66650);

	PLANCK_UNIT_ASSERT_TRUE(tc, err_ok == dict->last_status.error);
	PLANCK_UNIT_ASSERT_TRUE(tc, 1 == dict->last_status.count);

	int j = memcmp(GTEST_DATA, ret_val, dict->size_v);
...
test_cpp_wrapper_open_close_on_all_implementations(
	...
	dict = new BppTree<int, char*>(key_type_numeric_signed, sizeof(int), 50);
	test_cpp_wrapper_open_close(tc, dict);
	delete dict;

//	dict = new SkipList<int, char*>(key_type_numeric_signed, sizeof(int), 10, 7);
//	test_cpp_wrapper_open_close(tc, dict);
//	delete dict;

//	dict = new FlatFile<int, char*>(key_type_numeric_signed, sizeof(int), 10);
//	test_cpp_wrapper_open_close(tc, dict);
//	delete dict;

//	dict = new OpenAddressHash<int, char*>(key_type_numeric_signed, sizeof(int), 10, 50);
//	test_cpp_wrapper_open_close(tc, dict);
//	delete dict;

//	dict = new OpenAddressFileHash<int, char*>(key_type_numeric_signed, sizeof(int), 10, 50);
//	test_cpp_wrapper_open_close(tc, dict);
//	delete dict;
}
