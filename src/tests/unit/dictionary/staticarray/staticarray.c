...
void test_insert_collision(CuTest *tc) {
	...
	printf(
			"More Information was inserted with the same key, '%s'\nThe error was caught and the "
...
void test_delete_valid_case(CuTest *tc) {
	...
	CuAssertTrue(tc, strcmp((char*)b[k].value, ""));
...
void test_get_valid_case(CuTest *tc) {
	...
	status_t result = sa_insert(&st, key, value);
	printf(
			"Information was inserted with the key of '%s' and a value of \n '%s'\n",
			key, value);
	CuAssertTrue(tc, result == status_ok);
	...
	CuAssertTrue(tc, strcmp((char*)stored_data,(char*) value) == 0);
...
void test_get_invalid_key(CuTest *tc) {
	...
	printf(
			"The index location of %llu tried to be accessed but the max index is %llu\n\n",
			k, array_size);
...
void test_get_from_empty_location(CuTest *tc) {
	...
	printf(
			"A value was looked for at index %llu but none was found and the error was caught\n\n",
			k);
...
void test_update_invalid_key(CuTest *tc) {
	...
	printf(
			"index %llu was attempted to be accessed with a max array size of %llu \n",
			k, array_size);
	printf("The error was caught\n\n");
	status_t update_result = sa_update(&st, key, new_value);
...
void test_update_empty_location(CuTest *tc) {
	...
	printf(
			"the value '%s' was inserted into a blank location using the update method\n",
			new_value);
	status_t update_result = sa_update(&st, key, new_value);
...
void show_for_raymon(CuTest *tc) {
	...
	printf(
			"Please enter a array size to work with. (A array size of 0 will make a max length array\n");
	scanf("%d", &array_size);

	printf("Please enter a value size to work with\n");
	scanf("%d", &value_size);

	unsigned char user[value_size];
	unsigned char key[value_size];
	...
	scanf("%s", user);
	while (strcmp((char*)user, "quit") != 0) {
		if (strcmp((char*)user, "i") == 0) {
		...
		} else if (strcmp((char*)user, "d") == 0) {
			printf(
					"Enter a key that holds the information you want to delete\n");
			scanf("%s", key);
			sa_delete(&st, key);
			printf("The information was deleted\n");
		} else if (strcmp((char*)user, "u") == 0) {
		...
		} else if (strcmp((char*)user, "p") == 0) {
			int x;

			for (x = 0; x < st.array_size; x++) {
				sa_get(&st, (unsigned char *) &x, &v);
...
void runalltests_open_address_staticarray() {
