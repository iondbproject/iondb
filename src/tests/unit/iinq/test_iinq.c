...
iinq_test_create_query_select_all_from_where_single_dictionary(
	...
	QUERY(
		SELECT_ALL,
		FROM(0, test),
		WHERE(NEUTRALIZE(test.key, uint32_t) == 1),
		, , , , ,
		&processor
	);
...
iinq_test_create_query_select_expression_all_from_where_single_dictionary(
	...
	QUERY(
		SELECT(SELECT_EXPR(NEUTRALIZE(test.value, uint32_t) * 2)),
		FROM(0, test),
...
iinq_test_create_query_select_all_from_where_two_dictionaries(
	...
	QUERY(
		SELECT_ALL,
		FROM(0, test1, test2),
		WHERE(NEUTRALIZE(test1.key, uint32_t) == 1),
		, , , , ,
		&processor
	);
...
void
iinq_test_create_query_select_max_from_where_groupby_aggregate(
	planck_unit_test_t *tc
) {
	ion_err_t					error;
	ion_iinq_query_processor_t	processor;
	...
	MATERIALIZED_QUERY(
		...
		FROM(0, test),
		WHERE(1),
		GROUPBY(ASCENDING_INT(NEUTRALIZE(test.key, uint32_t)))
		,
		HAVING_ALL,
		ORDERBY_NONE
		, , ,
...
iinq_test_create_query_select_count_from_where_groupby_aggregate(
	...
	MATERIALIZED_QUERY(
		SELECT(SELECT_AGGR(0)),
		AGGREGATES(COUNT(NEUTRALIZE(test.key, uint32_t))),
		FROM(0, test),
		WHERE(1),
		GROUPBY(ASCENDING_INT(NEUTRALIZE(test.key, uint32_t)))
		,
...
iinq_test_create_query_select_sum_from_where_groupby_aggregate(
	...
	MATERIALIZED_QUERY(
		SELECT(SELECT_AGGR(0)),
		AGGREGATES(SUM(NEUTRALIZE(test.value, uint32_t))),
		FROM(0, test),
		WHERE(1),
		GROUPBY(DESCENDING_INT(NEUTRALIZE(test.key, uint32_t)))
		,
		HAVING_ALL,
		ORDERBY_NONE
		, , ,
...
void
iinq_test_create_query_select_max_from_where_groupby_aggregate_orderby_records(
	planck_unit_test_t	*tc,
	ion_iinq_result_t	*result,
	uint32_t			count,
	uint32_t 			total
...
void
iinq_test_create_query_select_max_from_where_groupby_aggregate_orderby(
	planck_unit_test_t *tc
) {
	ion_err_t					error;
	ion_iinq_query_processor_t	processor;
	...
	state.tc					= tc;
	state.total					= 2;
	state.func					= iinq_test_create_query_select_max_from_where_groupby_aggregate_orderby_records;
	...
	MATERIALIZED_QUERY(
		...
		GROUPBY(ASCENDING_INT(NEUTRALIZE(test.key, uint32_t))),
		HAVING_ALL,
		ORDERBY(DESCENDING_INT(AGGREGATE(0))),
		, ,
		&processor
	);
...
iinq_test_create_query_select_string_from_join_where_schema(
	...
	MATERIALIZED_QUERY(
		SELECT(SELECT_EXPR(test1_tuple->x), SELECT_EXPR(test2_tuple->z)),
		AGGREGATES_NONE,
		FROM(1, test1, test2),
		WHERE(0 == strncmp(test1_tuple->y, test2_tuple->y, 4)),
		GROUPBY_NONE,
		HAVING_ALL,
		ORDERBY_NONE,
		, ,
...
iinq_test_the_ultimate_query(
	...
	MATERIALIZED_QUERY(
		...
		AGGREGATES(MIN(test3_tuple->e), COUNT(test2_tuple->d), SUM(test2_tuple->d), MAX(test3_tuple->e)),
		FROM(1, test1, test2, test3),
		WHERE(test1_tuple->a == test2_tuple->c && 0 == strncmp(test2.key, test3_tuple->f, 5) && test3_tuple->e > 1000),
		GROUPBY(ASCENDING_UINT(test1_tuple->b), ASCENDING(test3_tuple->f)),
		HAVING((int16_t) AGGREGATE(2) % 2 == 0),
		ORDERBY(DESCENDING_UINT(test1_tuple->b), ASC(AGGREGATE(3)))
		, , ,
		&processor
	);
...
//	PLANCK_UNIT_ADD_TO_SUITE(suite, iinq_test_create_query_select_complex_expression_from_where_aggregates);
	...
	PLANCK_UNIT_ADD_TO_SUITE(suite, iinq_test_create_query_select_max_from_where_groupby_aggregate);
...
//	PLANCK_UNIT_ADD_TO_SUITE(suite, iinq_test_create_query_select_key_from_where_groupby);
	...
	PLANCK_UNIT_ADD_TO_SUITE(suite, iinq_test_create_query_select_max_from_where_groupby_aggregate_orderby);
